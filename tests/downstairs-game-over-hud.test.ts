...
test('Game Over HUD reuses WellLifeBlock (not plain 生命 N / 樓層 N) and keeps reason + Replay', async () => {
  const well = await readFile(new URL('../app/downstairs-well.tsx', import.meta.url), 'utf8');
  const css = await readFile(new URL('../app/globals.css', import.meta.url), 'utf8');

  expect(well.includes("from './well-life-block'")).toBe(true);
  expect(well.includes('well-hud-over')).toBe(true);
  expect(well.includes('<WellLifeBlock')).toBe(true);
  expect(well.includes("label={zh ? '生命' : 'Life'}")).toBe(true);
  expect(well.includes('floor={depth}')).toBe(true);
  ...
  // Game Over path must not keep the old plain-text primary stats
