-- Downstairs (LAD) rematch: allow replay when current occupants remain (1–4),
-- without requiring the room to be full (max_players is always 4). Host must
-- still be a member so the host simulator role stays valid. Turn-based games
-- keep the existing full-seat rematch rule.
...
begin
  ...
  if v_room.game_slug = 'downstairs' then
    if v_members < 1 then
      raise exception 'Players must still be in the room to replay.' using errcode = 'P0001';
    end if;
    if not public.is_room_member(v_room.id, v_room.host_guest_id) then
      raise exception 'The host must still be in the room to replay.' using errcode = 'P0001';
    end if;
  elsif v_members < v_room.max_players then
