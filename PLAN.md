...
> Current status: production Tic-tac-toe vertical slice deployed on Vercel with a separate Supabase production project. Connect Four / 四子棋 is implemented in-repo (`CON-` rooms, CPU or human opponent) and needs the Connect Four migration on each target before it can be created. 打天九 is in the Playroom catalogue as Game 4 (`TGW-` rooms, 1–4 humans, table always 4 seats); lobby CPU fill and the playable deal loop are later issues. `prototype/` remains a disposable visual reference.
