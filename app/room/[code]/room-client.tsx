...
export function RoomClient({ code }: { code: string }) {
  ...
  const text = language === 'en' ? {
    ...
    unready: 'Unready', imReady: 'I’m ready', start: 'Start game →', startVsCpu: 'Start vs CPU →', startSolo: 'Start Solo well →',
    ...
    help: isDownstairs
      ? 'Start alone for a Solo well, or wait for friends. The host browser simulates the well; the server stores Checkpoints.'
    ...
    unready: '取消準備', imReady: '我準備好了', start: '開始遊戲 →', startVsCpu: '對戰電腦 →', startSolo: '開始單人井 →',
    waitingBoth: isDownstairs ? '等待玩家' : '等待兩位玩家準備', replay: '重玩一次', reportRoom: '舉報房間', cpu: '電腦',
    reportReason: '可選填原因', reported: '已舉報', report: '舉報',
    help: isDownstairs
      ? '可一人即開 Solo 井，亦可等朋友。房主瀏覽器模擬井況；伺服器只存 Checkpoint。'
  ...
  const downstairsState = isDownstairsRoomState(snapshot.room.state) ? snapshot.room.state : null;
  const board = snapshot.room.status === 'playing' || snapshot.room.status === 'finished'
    ? isDownstairs && downstairsState && downstairsState.checkpoint
      ? <DownstairsWell
      ...
      : isDownstairs
        ? <div className="waiting-mark waiting-mark-stairs">⇧<br />{language === 'en' ? 'Well ready' : '井已就緒'}</div>
    ...
    : <div className={`waiting-mark${isConnect ? ' waiting-mark-connect' : ''}${isDownstairs ? ' waiting-mark-stairs' : ''}`}>{isDownstairs ? '⇧' : isConnect ? '⬤ ⬤' : '× ○'}<br />{isDownstairs ? (language === 'en' ? 'Solo / Shared well' : '單人 / 共用井') : isConnect ? '⬤ ⬤' : '○ ×'}</div>;
