...
import type { RestorableKid, RestorableStair, RestorableWell } from './state';
...
export class Engine {
  ...
  restoreFromWell(well: RestorableWell, cfg: EngineConfig = {
    ...
    this.winner = null;
    const stairs = well.stairs as RestorableStair[];
    const kids = well.kids as RestorableKid[];
    this.floors = stairs.map((s) => ({
      id: s.id,
      x: s.x,
      y: s.y,
      w: s.w,
    ...
    this.players = kids.map((kid, index) => {
